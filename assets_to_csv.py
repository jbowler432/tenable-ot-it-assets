...
def get_vuln_summaries(ip,uuid,repo,vulns):
	...
	med=0
	low=0
	url="https://"+sc+":443/rest/analysis"
	headers={
		'accept': "application/json",
		'content-type': "application/json",
		'X-SecurityCenter': mytoken
		}
	if uuid=='':
	...
	response=requests.request("POST",url,headers=headers,data=json.dumps(mydata),cookies=mycookies,verify=False)
...
sc=keys["server"] # enter you IP here
uname=keys["user"] # enter your T.sc username here
pword=keys["password"] # enter your T.sc password here

url="https://"+sc+":443/rest/token"
headers={
	'accept': "application/json",
	'content-type': "application/json"
	}
...
if pull_vuln_summaries==1:
	...
	for k,v in asset_info.items():
		...
		count=count+1
#
save_results(asset_info,output_file)
save_results_json(asset_info,output_file2)
#
...
response=requests.request("DELETE",url,headers=headers,cookies=mycookies,verify=False)
decoded=json.loads(response.text)
